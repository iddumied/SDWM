...
void setup_sbar()
{
  ...
  if(!tbarloaded){
    ...
    if(text_bar){
      ...
      bh = dc.h = sbars[ab].h;
      topbar = sbars[ab].topbar;
      sbarloaded = True;
      // setup status calculating
      setup_status();
  ...
  for(i = 0;i < ANZ_STYL_BARS;i++)
  {    
    sbars[i].w = screenWidth;
    sbars[i].h = confbar[i].h;
    sbars[i].ntabs = confbar[i].ntabs;
    sbars[i].topbar = confbar[i].topbar;
    
    // create a pixmap from the XPM file 
    status = XpmReadFileToPixmap(dpy, root, confbar[i].bg, 
				     &sbars[i].drawable, NULL, NULL);
	
    if (status != XpmSuccess) {
      fprintf(stderr, "line %d in i: %d XpmError: %s\n", 31, i, XpmGetErrorString(status));
      exit(1);
    }
    
    sbars[i].tabs = (STab*)malloc(sizeof(STab) * confbar[i].ntabs);
    
    for(j = 0;j < confbar[i].ntabs;j++)
    {
      sbars[i].tabs[j].w = confbar[i].tabs[j].w;
      sbars[i].tabs[j].h = confbar[i].tabs[j].h;
      sbars[i].tabs[j].pos_x = confbar[i].tabs[j].pos_x;
      sbars[i].tabs[j].pos_y = confbar[i].tabs[j].pos_y;
      
      // load has window tab xpm
      status = XpmReadFileToPixmap(dpy, root, confbar[i].tabs[j].has_win, 
				     &sbars[i].tabs[j].has_win , NULL, NULL);
	
      if (status != XpmSuccess) {
        fprintf(stderr, "line %d in i,j: %d,%d xpm: %s XpmError: %s\n", 52, i, j, confbar[i].tabs[j].has_win,  XpmGetErrorString(status));
        exit(1);
      }
      
      // load has active window tab xpm
      status = XpmReadFileToPixmap(dpy, root, confbar[i].tabs[j].has_activ_win, 
				   &sbars[i].tabs[j].has_activ_win , NULL, NULL);
	
      if (status != XpmSuccess) {
        fprintf(stderr, "line %d in i,j: %d,%d xpm: %s XpmError: %s\n", 61, i, j, confbar[i].tabs[j].has_activ_win,  XpmGetErrorString(status));
        exit(1);  
      }
      
      // load active empty tab xpm
      status = XpmReadFileToPixmap(dpy, root, confbar[i].tabs[j].empty_active, 
				   &sbars[i].tabs[j].empty_active , NULL, NULL);
	
      if (status != XpmSuccess) {
        fprintf(stderr, "line %d in i,j: %d,%d xpm: %s XpmError: %s\n", 70, i, j, confbar[i].tabs[j].empty_active,  XpmGetErrorString(status));
        exit(1);
      }  
      
      // load active full tab xpm
      status = XpmReadFileToPixmap(dpy, root, confbar[i].tabs[j].full_active, 
				   &sbars[i].tabs[j].full_active , NULL, NULL);
	
      if (status != XpmSuccess) {
        fprintf(stderr, "line %d in i,j: %d,%d xpm: %s XpmError: %s\n", 79, i, j, confbar[i].tabs[j].full_active,  XpmGetErrorString(status));
        exit(1);
      }
    }
  }
...
void togglesbar_mode()
{
  text_bar = !text_bar;
  if(text_bar){
    bh = dc.h = dc.font.height + 2;
    if(bottomborder)bh++;
    selmon->topbar = texttopbar;
  }else{
    if(!sbarloaded) setup_sbar();
    bh = dc.h = sbars[ab].h;
    selmon->topbar = sbars[ab].topbar;
  }
